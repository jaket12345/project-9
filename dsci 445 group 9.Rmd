...
To look for trends and similarities in tracks that have at least 
one billion streams (on Spotify). \ 
To look for trends and changes in popular Rap music by 
decade and as a whole. \ 
Looking for trends in different genres and comparing them 
to each other. \ 
...
## DATA SET 1: TRENDS IN BILLION - STREAMED TRACKS
...
## DATA SET 2 : 
Research Question: How was Rap changed since it's creation in the 70's?
...
```{r}
# MLR model
music.lm <- lm(Popularity ~ ., data = rap[,4:15]) # modeling all variables against Popularity
# Model Summary
summary(music.lm)
```
